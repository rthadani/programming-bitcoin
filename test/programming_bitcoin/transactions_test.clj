(ns programming-bitcoin.transactions-test
 (:require [midje.sweet :refer :all]
            [programming-bitcoin.helper :refer :all]
            [programming-bitcoin.transactions :refer :all]
            [programming-bitcoin.script :refer :all]))

(let [tx (clojure.string/replace "010000000456919960ac691763688d3d3bcea9ad6ecaf875df5339e

df2b3eda8db57397088ac46430600" #"\n" "")
      stream (clojure.java.io/input-stream (buddy.core.codecs/hex->bytes tx))
      parsed-tx (parse-tx stream)]
  (fact "Chapter - 6 - Exercise 5 - Script Sig"
        (to-string (:script-sig ((:tx-ins parsed-tx) 1))) => "304402207899531a52d59a6de200179928ca900254a36b8dff8bb75f5f5d71b1cdc26125022008b422690b8461cb52c3cc30330b23d574351872b7c361e9aae3649071c1a71601 035d5c93d9ac96881f19ba1f686f15f009ded7c62efe85a872e6a19b43c15a2937 ")
  (fact "Chapter-6 - Exercise 5 - Script Pubkey"
        (-> (:tx-outs parsed-tx)
            (first)
            (:script-pubkey)
            (to-string)) => "OP_DUP OP_HASH160 ab0c0b2e98b1ab6dbf67d4750b0a56244948a879 OP_EQUALVERIFY OP_CHECKSIG "))

(fact "Chapter 7 - Am i creating money"
      (let [raw-tx (buddy.core.codecs/hex->bytes "")
            stream (clojure.java.io/input-stream raw-tx)
            transaction (parse-tx stream)]
        (>= (fee transaction) 0) => truthy))